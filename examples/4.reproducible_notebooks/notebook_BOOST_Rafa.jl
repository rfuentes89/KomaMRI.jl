...
begin
	hbmp = 50:10:100
	RRs = 60 ./ (hbmp)  # RR [s]
    mag_1hb = zeros(ComplexF64, im_segments, Niso*4, length(RRs))
	mag_2hb = zeros(ComplexF64, im_segments, Niso*4, length(RRs))
    @progress for (m, RR) = enumerate(RRs)
		...
		mag_1hb[:, :, m] .= magaux[ end-2im_segments+1:end-im_segments, :]
		mag_2hb[:, :, m] .= magaux[end-im_segments+1:end, :]
    end
end

# ╔═╡ 389fdc31-3926-4e3a-98e6-67c68bf253bf
...
begin
    TIs = (60:10:150) # Inversion delay [ms]
...
begin
	...
	signal_1hb_myoc = reshape(
	    mean(abs.(mean(mag_1hb[:, spins[1], :],3)), 1), 1, length(RRs)
	)
	signal_1hb_bloo = reshape(
	    mean(abs.(mean(mag_1hb[:, spins[2], :],3)), 1), 1, length(RRs)
	)
	signal_1hb_fat = reshape(
	    mean(abs.(mean(mag_1hb[:, spins[3], :],3)), 1), 1, length(RRs)
	)
...
# ╔═╡ f1c18edd-c3fa-4c16-acfb-8268a5a6bb65
begin
	# Reducing tissues' signal
	signal_2hb_myoc = reshape(
	    mean(abs.(mean(mag_2hb[:, spins[1], :],3)), 1), 1, length(RRs)
	)
	signal_2hb_bloo = reshape(
	    mean(abs.(mean(mag_2hb[:, spins[2], :],3)), 1), 1, length(RRs)
	)
	signal_2hb_fat = reshape(
	    mean(abs.(mean(mag_2hb[:, spins[3], :],3)), 1), 1, length(RRs)
	)
	signal_2hb_diff = abs.(signal_2hb_myoc .- signal_2hb_bloo)
...
# ╔═╡ 5d170296-e794-45cb-b8ca-09e38250d1fd
begin
	# Substracted
	signal_bb_myoc = abs.(signal_1hb_myoc .- signal_2hb_myoc)
	signal_bb_bloo = abs.(signal_1hb_bloo .- signal_2hb_bloo)
	signal_bb_fat = abs.(signal_1hb_fat .- signal_2hb_fat)
	signal_bb_diff = abs.(signal_1hb_diff .- signal_2hb_diff)
	...
	# Plots
	fig_bb = plot([s1_3, s2_3, s3_3, s4_3])
	relayout!(
	    fig_bb;
	    yaxis=attr(; title="Signal [a.u.]", tickmode="array"),
	    xaxis=attr(;
...
manifest_format = "2.0"
project_hash = "1ea7133f24de8c93cb9214e8a73cbc6da1912ec8"
...
git-tree-sha1 = "2ae3627363b02a6f33ccc3495d0d9a11966ea680"
...
git-tree-sha1 = "74747c9914d27c52964c326c2ecb3256c22890a2"
...
git-tree-sha1 = "e70a6308ee953922403400e6f6c53be56ef0c2f7"
